...
public:
    vector<vector<string>> findLadders (string beginWord, string endWord, vector<string>& wordList) {
        ...
        vector<int> queue;
        vector<set<int>> path(size+2);
        for (int i = 0; i < size; ++i)
            if (isTransformable(beginWord, wordList[i])) {
                if (i == targetIndex) return {{beginWord, wordList[i]}};
                dp[i] = 2;
                path[2].insert(i);
                queue.push_back(i);
            }
        if (map[targetIndex].empty()) return {};
        ...
        while (!queue.empty()) {
            ...
            for (auto&cur:queue) {
                for (auto&next:map[cur]) {
                    if (next == targetIndex) found = true;
                    if (dp[next] == -1) {
                        dp[next] = curStep;
                        path[curStep].insert(next);
        ...
        vector<vector<int>> res_int = {{targetIndex}};
        for (int i = curStep-1; i > 1; --i) {
            vector<vector<int>> tmpRes;
            for (auto & v : res_int) {
                for (auto & n : map[v.back()]) {
                    if (path[i].find(n) != path[i].end()){
                        tmpRes.push_back(v);
...
int main() {
    ...
    for (auto&v:sol.findLadders (beginWord, endWord, wordList)) {
        for (auto&s:v)
            cout << s << " ";
        cout << endl;
    }
    ...
    // cout << sol.findLaddersk (beginWord, endWord, wordList) << endl;
    // beginWord = "hog";
    // endWord   = "cog";
    // wordList = {"cog"};
    // cout << sol.findLaddersk (beginWord, endWord, wordList) << endl;
